 ...
 *
 * 断言以京房纳甲、八宫世应通行规则为基准。
 * 六爻公开入口仅限铜钱；数字/时间入口通过 _internal 旁路供内部算法测试。
 */
...
// 内部测试辅助：指定数字起卦（通过 _internal 旁路）
function panByNum(num1, num2, num3) {
  return liuyao.panGua('number', { _internal: true, num1: num1, num2: num2, num3: num3, dt: new Date(2026, 5, 28, 14, 30) });
}

runner.module('六爻 API 与输入边界');
runner.test('核心 API 全部存在', function() {
  runner.assert(liuyao && typeof liuyao.panGua === 'function');
  ['qiGuaByTime','qiGuaByNumber','qiGuaByCoin','tossCoin'].forEach(function(name) {
    runner.assert(typeof liuyao[name] === 'function', name + ' 应存在');
  });
  // qiGuaByRandom / qiGuaByYarrow 已从六爻页移除
  runner.assert(typeof liuyao.qiGuaByRandom === 'undefined', 'qiGuaByRandom 应已删除');
});
runner.test('panGua 仅支持 coin；其他 method + 无 _internal 时报错', function() {
  assertThrows(function() { liuyao.panGua('time', { dt: new Date() }); }, '六爻已仅支持铜钱摇卦');
  assertThrows(function() { liuyao.panGua('number', { num1:1, num2:1, num3:1, dt: new Date() }); }, '六爻已仅支持铜钱摇卦');
  assertThrows(function() { liuyao.panGua('coin', { dt: new Date('invalid') }); }, '日期');
  // _internal 旁路正常
  var pan = liuyao.panGua('time', { _internal: true, dt: new Date(2026, 5, 28, 14, 30) });
  runner.assert(pan.gua && pan.gua.name, '_internal time 应正常工作');
});
...
runner.test('panGua coin 结构完整（六爻页唯一公开入口）', function() {
  var pan = liuyao.panGua('coin', { dt: new Date(2026,5,28,14,30) });
  runner.assert(pan.gua && pan.gua.name, '应有卦名');
  runner.assertEq(pan.yaoList.length, 6, '六爻');
  runner.assert(pan.timeGanzhi, '四柱');
  runner.assert(pan.lunarText, '农历文本');
  runner.assertEq(pan.method, '铜钱摇卦');
});

runner.module('京房纳甲、八宫六亲与世应');
runner.test('乾为天：内甲子寅辰、外壬午申戌', function() {
  var pan = panByNum(1, 1, 1);
...
runner.test('天地否与水雷屯的上下卦纳甲正确', function() {
  var pi = panByNum(1, 8, 1);
  var tun = panByNum(6, 4, 1);
...
runner.test('全64卦、384种单爻变化满足线序不变量', function() {
  var baseByName = {};
  for (var u=1; u<=8; u++) for (var l=1; l<=8; l++) {
    var base = panByNum(u, l, 1);
  ...
  Object.keys(baseByName).forEach(function(name) {
    ...
    var sample = panByNum(pair[0], pair[1], 1);
...
runner.module('铜钱 coin 路径完整覆盖');
runner.test('panGua coin 结构完整（大衍等入口已从六爻页移除，此处验证 coin 路径）', function() {
  var pan = liuyao.panGua('coin', { dt: new Date(2026,5,28,14,30) });
  runner.assert(pan.gua && pan.gua.name.length >= 3, '应有卦名');
  runner.assertEq(pan.yaoList.length, 6);
  runner.assertEq(pan.method, '铜钱摇卦');
  runner.assert(pan.coinResults && pan.coinResults.length === 6, '铜钱结果记录');
});
